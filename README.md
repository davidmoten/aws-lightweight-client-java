...
When you use API Gateway integrated with Lambda, say as a JSON service, what is the cost of the integration plus the lambda runtime? This is a particularly relevant question if you want to use a setup like this for something user-facing. Across 25 cold start samples with the Lightweight client the average request time was 1.995s (stdev 0.273s) which corresponds to an average cold-start overhead from APIG integration of 0.952s. I'm not sure why the overhead figure is that high. However, for some of my use cases an occasional cold-start request time of 2s is acceptable and I don't need to keep my Lambdas warm (using another scheduled Lambda for instance).

Warm start sample request times were on average 0.25s.

Note that these tests were done from my not-very-zippy home internet connection. The ping latency to AWS ap-southeast-2 is ~35ms.
