...
public class STSCredentialsProviderTest {
    ...
    public void testAssumeRoleWithWebIdentityIOException() {
        QueueHttpClient httpClient = new QueueHttpClient();
        ...
        Map<String, String> map = Maps //
                ...
                .buildImmutable();
        Environment env = map::get;
        STSCredentialsProvider.assumeRoleWithWebIdentity(httpClient, env,
                "token", "arn:aws:iam::123456789012:role/test", "session");
    }

    @Test
    public void testStsEndpointRegionalViaSystemProperty() {
        System.setProperty("aws.stsRegionalEndpoints", "regional");
        try {
            Map<String, String> map = Maps //
                    .put("AWS_REGION", "eu-central-1") //
                    .buildImmutable();
            Environment env = map::get;
            assertEquals("https://sts.eu-central-1.amazonaws.com/",
                    STSCredentialsProvider.stsEndpoint(env));
        } finally {
            System.clearProperty("aws.stsRegionalEndpoints");
        }
    }

    @Test
    public void testSessionNameFromSystemProperty() {
        System.setProperty("aws.roleSessionName", "sys-session-name");
        try {
            long baseTime = 1700000000000L;
            SettableClock clock = new SettableClock(baseTime);
            String expiration = Instant.ofEpochMilli(baseTime + 3600_000).toString();
            ...
            Map<String, String> map = Maps //
                    ...
                    .put("AWS_WEB_IDENTITY_TOKEN_FILE", "src/test/resources/test.txt") //
                    .put("AWS_ROLE_ARN", "arn:aws:iam::123456789012:role/test-role") //
                    .put("AWS_ROLE_SESSION_NAME", "ignored-session-name") //
                    .buildImmutable();
            Environment env = map::get;
            Optional<Credentials> result = STSCredentialsProvider.credentials(env,
                    httpClient, clock);
            ...
            System.clearProperty("aws.roleSessionName");
        }
    }

    @Test
    public void testSessionNameAutoGenerated() {
        long baseTime = 1700000000000L;
        SettableClock clock = new SettableClock(baseTime);
        String expiration = Instant.ofEpochMilli(baseTime + 3600_000).toString();
        String xml = stsResponseXml("KEY1", "secret1", "token1", expiration);
        ...
        // Don't set ROLE_SESSION_NAME - should auto-generate
        Map<String, String> map = Maps //
                .put("AWS_STS_ENDPOINT_URL", "http://localhost/test") //
                .put("AWS_WEB_IDENTITY_TOKEN_FILE", "src/test/resources/test.txt") //
                .put("AWS_ROLE_ARN", "arn:aws:iam::123456789012:role/test-role") //
    ...
    @Test
    public void testAutoRefreshCoversAllAccessorMethods() {
        long baseTime = 1700000000000L;
        SettableClock clock = new SettableClock(baseTime);
        String expiration = Instant.ofEpochMilli(baseTime + 3600_000).toString();
        String xml = stsResponseXml("KEY1", "secret1", "token1", expiration);
    ...
    private static final class QueueHttpClient implements HttpClient {
        ...
        void enqueue(int statusCode, String body) {
            queue.add(new ResponseEntry(statusCode, body, null));
        }

        void enqueueError(IOException error) {
            queue.add(new ResponseEntry(0, null, error));
        }
        ...
        private static final class ResponseEntry {
            ...
            ResponseEntry(int statusCode, String body, IOException error) {
